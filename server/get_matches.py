...
def calculate_compatibility(A, B, preferences={}):
    ...
    B["dislikes"] = calc_dislike_value(B["age"], B["gender"], B["pronouns"], preferences)
...
def get_n_users(n, username):
    ...
    c.execute("SELECT user_name, age, gender, pronouns, lat, lng FROM users WHERE ROWID >= ? AND user_name!=? LIMIT ?", (n, username, index))
    users = c.fetchall()
...
def get_n_sorted_users(n, username):
    ...
    c.execute("SELECT user_name, age, gender, pronouns, lat, lng FROM users WHERE user_name=?", (username,))
    user_A = c.fetchone()

    c.execute("SELECT interest1, interest2, interest3, interest4, interest5 FROM interests WHERE user_name=?", (user_A[0],))
    interests = c.fetchone()
    user_A = {"user_name": user_A[0], "age": user_A[1], "gender": user_A[2], "pronouns": user_A[3], "interests": interests, "lat": user_A[4], "lng": user_A[5]}

    users = get_n_users(n, username)

    for i in range(len(users)):
        c.execute("SELECT interest1, interest2, interest3, interest4, interest5 FROM interests WHERE user_name=?", (users[i][0],))
        interests = c.fetchone()
        users[i] = {"user_name": users[i][0], "age": users[i][1], "gender": users[i][2], "pronouns": users[i][3], "interests": interests, "lat": users[i][4], "lng": users[i][5]}
