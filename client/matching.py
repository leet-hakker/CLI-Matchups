...
matches = [
    {
        ...
        "compatibility": 0,
        "likes": 0,
        "interests": random.randrange(6),
        "dislikes": 0,
        "latitude": random.uniform(47.2921, 49.2921),
        "longitude": random.uniform(14.6335, 16.6335),
    }
    for i in range(100)
]
...
def partition(arr, l, h):
    ...
    for j in range(l, h):
        if arr[j]["compatibility"] <= x:
            ...
            i = i + 1
            arr[i]["compatibility"], arr[j]["compatibility"] = (
                arr[j]["compatibility"],
                arr[i]["compatibility"],
            )

    arr[i + 1]["compatibility"], arr[h]["compatibility"] = (
        arr[h]["compatibility"],
        arr[i + 1]["compatibility"],
    )
    return i + 1
